{def $page_limit = 10
      $all_latest_content_count = fetch( 'content', 'tree_count', hash( 'parent_node_id', $sensor.forum_container_node.node_id,
                                                                        'class_filter_type', 'include',
                                                                        'class_filter_array', array( 'dimmi_forum_reply' ),
                                                                        'attribute_filter', array( array( 'owner', '=', $current_user.contentobject_id ) ),
                                                                        'sort_by', array( 'published', false() ) ) )
      $all_latest_content = fetch( 'content', 'tree', hash( 'parent_node_id', $sensor.forum_container_node.node_id,
                                                                  'limit', $page_limit,
...
<section class="hgroup">
  <div class="row">
    <div class="col-md-12">
      <h1>
        {"Le mie discussioni"|i18n('openpa_sensor/dashboard')}
      </h1>
    </div>
  </div>
</section>
...
{else}
  <em>Nessuna discussione trovata... Inizia da <a href={"sensor/dimmi"|ezurl()}>qui</a></em>
{/if}
